...
/**
 * The internal MIME type used with the `DataTransfer` attribute of `DragEvent`.
 */
const DRAG_MIME_TYPE = 'application/x-phosphor-internal-drag-data';


/**
 * The internal cache that holds data for drag events and the id for the current
 * DragEvent.
 */
var dragCache = {
 ...
 */
export
function getDragData(event: DragEvent, mime: string): any {
 ...
 */
export
function setDragData(event: DragEvent, mime: string, data: any): void {
  let id = event.dataTransfer.getData(DRAG_MIME_TYPE);
  if (!id) {
...
/**
 * Clear data being held for a specific drag event.
 *
 * @param event - The DragEvent generated by a user interaction.
 *
 * #### Notes
